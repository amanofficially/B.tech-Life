  ...
  setInterval(() => {
    if (
      ...
      document.body.innerHTML =
        '<div style="display:flex;height:100vh;align-items:center;justify-content:center;font-family:sans-serif;font-size:1.4rem;color:#3d8de8;background:#fdfaf6">🔒 This page is protected.</div>';
    }
  }, 1000);
})();
...
const cIO = new IntersectionObserver(
  (entries) => {
    entries.forEach((e) => {
      ...
      const el = e.target,
        target = parseInt(el.dataset.to, 10);
      let n = 0;
      const steps = 60,
        inc = target / steps;
      const t = setInterval(() => {
...
/* ── 10. TIMELINE FLIP — TOUCH/MOBILE TAP SUPPORT ── */
...
/* ── 13. QUOTE CARD STAGGER ON SCROLL ── */
...
/* ── 15. SMOOTH ACTIVE NAV LINK HIGHLIGHT ── */
...
/* ── 16. TIMELINE NODE ANIMATE ON SCROLL ── */
      ...
      if (e.isIntersecting) {
        ...
        if (node) {
          node.style.animation =
            "nodeEntrance .6s cubic-bezier(.34,1.56,.64,1) forwards";
        }
      }
    });
  },
...
/* Inject dynamic keyframes + nav active style */
